...
func (f *Crdclient) Create(obj *vtype.Lhvolume) (*vtype.Lhvolume, error) {
	var result vtype.Lhvolume
	err := f.cl.Post().
...
func (f *Crdclient) Update(obj *vtype.Lhvolume) (*vtype.Lhvolume, error) {
	var result vtype.Lhvolume
	err := f.cl.Put().
		Namespace(f.ns).Resource(f.plural).
		Body(obj).Do().Into(&result)
...
func (f *Crdclient) Get(name string) (*vtype.Lhvolume, error) {
	var result vtype.Lhvolume
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
...
func (f *Crdclient) List(opts meta_v1.ListOptions) (*vtype.LhvolumeList, error) {
	var result vtype.LhvolumeList
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
